...
public class FormatSew implements IFormat.Reader, IFormat.Writer {
...
public void read(EmbPattern pattern, InputStream stream) {
    ...
    try {
        ...
        while (true) {
            ...
            int   stitch_type = globals.STITCH_NORMAL;
            if (stream.read(b) != 2) {
                break;
            }
            if (((b[0] & 0xFF) == 0x80)) {
                if ((b[1] & 0x01)  != 0) {
                    ...
                    flags = IFormat.STOP;
                    stitch_type = globals.STITCH_STOP;
                } else if ((b[1] == 0x04) || (b[1] == 0x02)) {
                    if (stream.read(b) != 2) {
                        break;
                    }
                    flags = IFormat.TRIM;
                    stitch_type = globals.STITCH_TRIM;
                } else if (b[1] == 0x10) {
                    break;
                }
            }
...
for(i = 2; i < 7538; i++)
{
BinaryHelper.writeInt32(stream, 0x00);
}
        ...
        for (EmbPoint stitches1 : pattern.getstitches1()) {
            ...
            encode(b, (byte) Math.round(dx), (byte) Math.round(-dy), flags); //swap y axos
