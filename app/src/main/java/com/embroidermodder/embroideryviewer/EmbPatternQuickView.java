...
public class EmbPatternQuickView {
    EmbPattern pattern;
    ...
    public EmbPatternQuickView(EmbPattern pattern) {
        this.pattern = pattern;
        BlockDraw draw = null;
        EmbThread lastThread = null;

        for (EmbObject sb : pattern.asStitchEmbObjects()) {
            EmbThread currentThread = sb.getThread();
    ...
    private class BlockDraw {
        ...
        public void addAsSegments(Points points) {
            ...
            for (int s = points.size() - 1; k < s; k++) {
                x = points.getX(k);
                y = points.getY(k);
                if (points.getData(k) != EmbPattern.STITCH) {
